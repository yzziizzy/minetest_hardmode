...
function campfire.fire_formspec(item_percent)
	local formspec =
		'size[8,6.75]'..
		default.gui_slots..
...
function burn(pointed_thing) --kindling doesn't always start from the first spark
	...
	if ignite_chance == 1
		and string.find(minetest.get_node(pointed_thing.under).name, 'campfire:kindling_contained')
		then
			minetest.set_node(pointed_thing.under, {name = 'campfire:embers_contained'})
	elseif ignite_chance == 1
		and string.find(minetest.get_node(pointed_thing.under).name, 'campfire:kindling')
		then
			minetest.set_node(pointed_thing.under, {name = 'campfire:embers'})
	else --Do nothing
	end
end
...
function smoke_particles(pos)
    minetest.add_particlespawner({
        ...
        texture = 'campfire_smoke.png', -- image to use (e.g. 'bubble.png' )
...
function lava_particles(pos)
	minetest.add_particlespawner({
        ...
        maxsize = 2,
        collisiondetection = false,
        texture = 'campfire_lava_blob.png',
        vertical = false,
--      playername = 'singleplayer',
    })
end
